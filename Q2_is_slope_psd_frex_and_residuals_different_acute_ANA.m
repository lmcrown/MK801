...
clear all
ANA_dir='F:\Keck Medicine of USC\MK801_Results\Acute\Q2_is_slope_psd_frex_and_residuals_different\PSD7';
...
for iF = 1:length(d)
    ...
    if strfind(st{2},'PRE')==1
        day(ses_cnt)= 0;
    elseif strfind(st{2},'POST')==1
        day(ses_cnt)=1;
    end
...
PREIX=day==0;
POSTIX=day==1;
%% Slope difference- not enough animals have both
diff=nan(length(animal),1);
for iani=1:length(an)
    ANIX=animal==an(iani);
    if sum(sum (ANIX & POSTIX) + sum(ANIX & PREIX)) ~=2
        continue
    end
    diff(iani)=slope(ANIX & POSTIX)-slope(ANIX & PREIX);
end
figure;
bar_plot_with_p_publish_updated(diff,POSTIX & SIX, POSTIX & MIX, 'Slope of PSD')
xticklabels({'Saline' 'MK801'})
%% Overall POST slope comparison
figure;
bar_plot_with_p_publish_updated(slope,POSTIX & SIX, POSTIX & MIX, 'Slope of PSD')
xticklabels({'Saline' 'MK801'})

figure
plot_confidence_intervals(frex,10*log10(pxx_no_art(POSTIX & SIX,:)),[],[0 0 1])
hold on
plot_confidence_intervals(frex,10*log10(pxx_no_art(POSTIX & MIX,:)),[],[1 0 0])
