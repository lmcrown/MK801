...
close all
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Analysis_function=@CFC_hipp_mpfc;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

root_results_directory = 'F:\Keck Medicine of USC\MK801_Results\Chronic';
...
root_data_directory = 'F:\Keck Medicine of USC\Zepeda, Nancy - 20946 ALL EEG FILES AGAIN\20946-Chron\EEG\20946-chronic-PSD7-23';
...
%     cd(tdir)
    ...
    for idir=1:length(drug2)
        drugdir=fullfile(root_data_directory,drug2(idir).name);
        cd(drugdir)
        day=dir(fullfile(root_data_directory,drug2(idir).name));
        good_ix = [];
        for ii = 1:length(day)
        ...
        for iday=1:length(day2)
            ...
            for irat=1:length(rats2)
                ...
                if Dset.aborted==false
                    dayresults=fullfile(root_results_directory, func2str(Analysis_function));
                    if ~exist(dayresults,'dir')
                        mkdir(dayresults)
                    end
                     if strcmp(da(1:3),'PSD')==1
                         daa=strcat('0', da(end));
                     else
                         daa=da(end-1:end);
                     end
                    save(fullfile(dayresults,[Dset.animal '_' daa]) ,'Dset') % i want to make a folder for the day
                end
                cd ..
            end
            cd ..
